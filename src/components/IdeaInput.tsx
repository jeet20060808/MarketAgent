...
export function IdeaInput({
  ...
  return (
    <section
      id="idea-input"
      className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-2xl shadow-black/20 backdrop-blur-xl sm:p-6 relative"
    >
      <button
        type="submit"
        form="idea-form"
        disabled={isLoading}
        className="absolute right-4 top-4 inline-flex h-12 items-center justify-center rounded-full bg-gradient-to-r from-cyan-400 to-emerald-400 px-5 text-sm font-semibold text-zinc-950 transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 z-10"
      >
